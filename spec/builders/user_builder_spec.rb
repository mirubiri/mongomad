...
describe UserBuilder do
	...
	describe '#gender' do
		specify { expect(builder.gender ':male' ).to eq builder }
	end

	describe '#language' do
		specify { expect(builder.language ':spanish' ).to eq builder }
	end

	describe '#birth_date' do
		specify { expect(builder.birth_date '15/10/1981' ).to eq builder }
	end

	describe '#location' do
		specify { expect(builder.location [1,2] ).to eq builder }
	end

	describe '#email' do
		specify { expect(builder.email 'an@email.es' ).to eq builder }
	end
