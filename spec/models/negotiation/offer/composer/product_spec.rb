...
describe Negotiation::Offer::Composer::Product do
  #let(:product) { Fabricate.build(:product) }
  #after(:each) { product && product.polymorphic_product.polymorphic_composer.destroy }

  describe 'Relations' do
    it { should be_embedded_in(:composer).of_type(Negotiation::Offer::Composer) }
    it { should embed_one(:secondary_image).of_type(Negotiation::Offer::Composer::Product::Image) }
  end
